...
local function StartFly()
    ...
    -- TP walking speed threads
    tpwalking=true
    for _=1,Cfg.Fly.Speed do
        task.spawn(function()
            local hb=RunService.Heartbeat
            while tpwalking and hb:Wait() and char and h and h.Parent do
                if h.MoveDirection.Magnitude>0 then
                    char:TranslateBy(h.MoveDirection)
                end
            end
        end)
    end
    ...
    task.spawn(function()
        ...
        while flyActive and char and h and h.Parent do
            ...
            flyBG.CFrame=camCF*CFrame.Angles(-math.rad((flyCtrl.f+flyCtrl.b)*50*flySpd/maxS),0,0)
        end
...
local function FireCollectE()
    ...
    -- Method 3: keypress() for executors that support it
    pcall(keypress, 0x45)  -- VK_E = 0x45

    -- Method 4: Scan ProximityPrompts and fire them
    local char=Char()
    if char then
        for _,v in pairs(workspace:GetDescendants()) do
            if v:IsA("ProximityPrompt") and v.Enabled then
                local vRoot=v.Parent:FindFirstChild("HumanoidRootPart")
                    or (v.Parent.Parent and v.Parent.Parent:FindFirstChild("HumanoidRootPart"))
                local myRoot=Root()
                if myRoot then
                    local checkPart=v.Parent:IsA("BasePart") and v.Parent or
                        (v.Parent:IsA("Model") and v.Parent:FindFirstChild("HumanoidRootPart"))
                    if checkPart then
...
RunService.Heartbeat:Connect(function()
    ...
    if tick()-lastCollect < Cfg.FastCollect.Delay/100 then return end
    lastCollect=tick()
    FireCollectE()
end)
...
local function SwitchTab(name)
    ...
    if name=="Combat" then
        ...
        Slider("Collect Speed","⏩",T.Lime,1,20,
            ...
            function(v) Cfg.FastCollect.Delay=v end," ms")
    ...
    elseif name=="Fly" then
        ...
        Slider("Fly Speed","🏎️",T.Sky,1,10,
            ...
            function(v)
                Cfg.Fly.Speed=v
                -- Restart fly threads with new speed if active
                if flyActive then
                    tpwalking=false
                    task.wait(0.05)
                    tpwalking=true
    ...
    elseif name=="Config" then
        ...
        SL.BackgroundTransparency=1 SL.TextColor3=T.Sub FL.Font=Enum.Font.Gotham SL.TextSize=10
...
print("╔══════════════════════════════════════════╗")
print("║   🥟 PELMEN PVP v4.1 — FLY + COLLECT   ║")
print("║   ✈️ Fly:      F key (PC) | Button (mob) ║")
print("║   🌿 Collect:  Auto E-spam + Proximity   ║")
